...
function calculateArea(xCoords, yCoords) {
  return 0.5 * Math.abs(calculateDotProduct(xCoords, transpose(yCoords)) - calculateDotProduct(yCoords, transpose(xCoords)));
}

function calculateDotProduct(xCoords, yCoords) {
  return xCoords.map((elem, index) => elem * yCoords[index]).reduce((acc, val) => acc + val);
}
...
for(let part of ['part1', 'part2']) {
  ...
  input.forEach(instruction => {
    ...
    if (part === 'part1') {
      ...
      currentDirection = directions[directionName[parseInt(hexValue[hexValue.length - 2])]];
      currentLength = parseInt(hexValue.substring(hexValue.length - 7, hexValue.length - 2), 16);
    }
    
    totalPoints+=currentLength;

    currentLocation = [currentLocation[0] + currentLength * currentDirection[0], currentLocation[1] + currentLength * currentDirection[1]];
